...
$(document).ready(function(){$.getJSON("http://jsonip.com/?callback=?", function (data){/*document.getElementById("ipPublicaID").innerHTML = '<p>Ip Publica: <b>'+data.ip+'</b>';*/ipExtraData(data.ip);proxyCheck2(data.ip);});});
...
function proxyCheck2(d){function ipExtraData(dd){$.get('https://ipstack.com/ipstack_api.php?ip='+d,function(ddd){console.log(ddd)},"jsonp");}};
