...
function generarSelectores(){
	try{
		variablesCSSStyleSheet=document.styleSheets[6].cssRules[0].style.cssText.split("--").join().split(";");
		
		variablesCSS=[];
		variablesCSSStyleSheet.forEach(v => {
			if(v.length>0){
				var variable=v.split(":")[0].trim().replace(',','');

				variablesCSS.push(variable);
			}
		});
	}catch(e){
